...
exports.register = async function (fastify, request, reply) {
    try {
        ...
        if (emailExists.length > 0) {
            return reply.code(400).send({ success: true, message: 'Account already exists.' });
        }

        const [usernameExists] = await fastify.mysql.query('SELECT * FROM users WHERE username = ?', [username]);
        if (usernameExists.length > 0) {
            return reply.code(400).send({ success: true, message: 'Username already exists.' });
        }
        ...
        if (result.affectedRows > 0) {
            return reply.code(201).send({ success: true, message: 'User account created successfully.' });
        } else {
            return reply.code(500).send({ success: false, message: 'Failed to create user account.' });
        }
    } catch (err) {
        console.error(err);
